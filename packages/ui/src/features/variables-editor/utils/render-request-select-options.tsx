...
export default function renderRequestSelectOptions(context: Context) {
	...
	return (
		<React.Fragment>
			{formattedNodes.filter(i => i.type === 'folder').map(i => renderFolder(i as FolderNode, context, depth))}
			{formattedNodes.filter(i => i.type === 'request').map(i => renderRequest(i as RequestNode, depth))}
		</React.Fragment>
	);
}
...
function renderFolder(node: FolderNode, context: Context, depth: number) {
	...
	return (
		<React.Fragment key={node.id}>
			<optgroup label={`${renderDepth(depth)}${node.name}`}>{nodes.map(n => renderRequest(n, newDepth))}</optgroup>
