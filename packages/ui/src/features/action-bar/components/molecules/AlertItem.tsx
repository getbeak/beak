...
const AlertItem: React.FC<AlertItemProps> = ({ title, description, action }) => (
	...
	>
		<Flex
			...
			mt='0.5'
			boxShadow='0 3px 8px color-mix(in srgb, var(--beak-colors-accent-warning) 18%, transparent), inset 0 1px 0 color-mix(in srgb, white 14%, transparent)'
		>
			<AlertTriangle size={12} strokeWidth={2.2} />
		</Flex>
		<Box flex='1 1 auto' minW={0}>
			<Box fontSize='sm' fontWeight='600' color='fg.default' letterSpacing='-0.005em' lineHeight='1.3'>{title}</Box>
			<Box fontSize='xs' color='fg.muted' mt='0.5' lineHeight='1.5'>{description}</Box>
		</Box>
		{action && (
