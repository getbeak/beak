...
const RequestTab: React.FC<RequestTabProps> = ({ flight }) => {
	...
	const tab = useAppSelector(s => s.global.preferences.requests[requestId]?.response.subTab.request) as Tab | undefined;
	...
	return (
		<Flex direction='column' overflow='hidden' h='100%'>
			...
			<Box flexGrow={2} overflowY='hidden' h='100%'>
				...
				{tab === 'raw' && <EditorView language='http' value={output} options={{ readOnly: true, wordWrap: 'on' }} />}
			</Box>
		</Flex>
	);
};
