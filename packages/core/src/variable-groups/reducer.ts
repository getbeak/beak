...
import { generateValueIdent, type VariableGroupsState } from './types';

/**
 * Attaches the pure variable-groups reducer cases to the given builder. The
 * builder's state type only needs to be a subtype of VariableGroupsState — UI
 * packages compose this into a wider state shape (rename, file-watch, etc.).
 */
export function buildVariableGroupsReducer<S extends VariableGroupsState>(builder: ActionReducerMapBuilder<S>) {
	builder
		...
		.addCase(actions.removeGroup, (state, { payload }) => {
			...
			TypedObject.keys(state.variableGroups[payload.id].values)
