# 0009 — Signed-challenge identity verification for the local agent
...
- Public keys in localStorage are not secrets; their exposure to XSS
  ...
  XSS-sensitive — that's the subject of [0010](0010-agent-token-storage.md).
...
- [0001](0001-local-agent-for-web-host.md) — original local agent
  design.
- [0010](0010-agent-token-storage.md) — token storage strategy
  (companion ADR).
