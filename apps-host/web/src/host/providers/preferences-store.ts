...
export default class WebPreferencesStore extends PreferencesStore {
	...
	async getEnvironment(): Promise<Environment> {
		// StorageProvider always falls back to defaults; cast is safe here.
	...
	async setEditorValue<Key extends keyof EditorPreferences>(key: Key, value: EditorPreferences[Key]): Promise<void> {
		...
		// Single-window: emit through the web IPC bus so the renderer listener fires.
		webIpcMain.emit('editor_preferences_updated');
	}
	...
	async setThemeMode(mode: ThemeMode): Promise<void> {
		...
		webIpcMain.emit('theme_mode_updated', mode);
	}
